...
You need [go version 1.15+](https://go.dev/) to run this project

- Clone the repo
- cd into repo directory
- Prepare report file (eg: `go run generate/main.go -out data/messages.1.data -verify data/verify.1.csv --seed 1560981440 -count 20`)
- Run the server, specifying the location to records file (`go run main.go --data-source=./data/messages.1.data`)
- Optionally run the frontend project to have a user-friendly interface


# How it works (summarization)

The summarisation step scans each record in the data source and prepares an in-memory DB to back the API server

The DB has an array of customers and a link map (think of DB index) to enhance direct access to a customer.
The DB also has an internaml map to ensure an event record is processed only once.

The preparation step works as described below.

General flow

![algorithim](./docs/general-flow.jpeg)

Register customer flow

![algorithim](./docs/register-user.jpeg)

Process record flow

![algorithim](./docs/process-record.jpeg)
